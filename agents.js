...
module.exports = async function handler(req,res){
  try{
    ...
    if(req.method==="GET"){
      const r=await sbRequest("GET",`/rest/v1/${encodeURIComponent(AGENT_TABLE)}?select=*&order=created_at.desc`);
      if(!r.ok) return json(res,r.status,{error:r.text});
      const agents=JSON.parse(r.text||"[]");
      for(const a of agents){
        const d=await sbRequest("GET",`/rest/v1/${encodeURIComponent(AGENT_DEVICE_TABLE)}?agent_id=eq.${encodeURIComponent(a.id)}&select=device_id`).catch(()=>({ok:false,text:"[]"}));
        a.device_count=d.ok?JSON.parse(d.text||"[]").length:0;
      }
    ...
    if(req.method==="POST"){
      ...
      if(action==="create"){
        ...
        if(!slug||!name||!code) return json(res,400,{error:"缺少中介编号、名称或授权码"});
        const payload={
          id:slug,
          slug,
          name,
        ...
        const r=await sbRequest("POST",`/rest/v1/${encodeURIComponent(AGENT_TABLE)}?on_conflict=id`,payload);
        if(!r.ok) return json(res,r.status,{error:r.text});
        return json(res,200,{ok:true});
      }
      if(action==="resetDevices"){
        const r=await sbRequest("DELETE",`/rest/v1/${encodeURIComponent(AGENT_DEVICE_TABLE)}?agent_id=eq.${encodeURIComponent(body.id)}`);
        if(!r.ok) return json(res,r.status,{error:r.text});
        return json(res,200,{ok:true});
      }
      if(action==="setActive"){
        const r=await sbRequest("PATCH",`/rest/v1/${encodeURIComponent(AGENT_TABLE)}?id=eq.${encodeURIComponent(body.id)}`,{is_active:!!body.active,updated_at:new Date().toISOString()});
        if(!r.ok) return json(res,r.status,{error:r.text});
        return json(res,200,{ok:true});
      }
      if(action==="delete"){
        await sbRequest("DELETE",`/rest/v1/${encodeURIComponent(AGENT_DEVICE_TABLE)}?agent_id=eq.${encodeURIComponent(body.id)}`).catch(()=>{});
        const r=await sbRequest("DELETE",`/rest/v1/${encodeURIComponent(AGENT_TABLE)}?id=eq.${encodeURIComponent(body.id)}`);
        if(!r.ok) return json(res,r.status,{error:r.text});
        return json(res,200,{ok:true});
      }
