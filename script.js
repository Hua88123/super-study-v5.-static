...
const LOCAL_BACKUP_KEY="super-study-local-backups-v1";
...
function schoolCountOf(d){
  return d && Array.isArray(d.schools) ? d.schools.length : 0;
}
function backupSnapshot(reason="自动备份", source){
  try{
    source = source || data;
    if(!source || !source.schools || !source.schools.length) return;
    const item = {
      id:"bk_"+Date.now()+"_"+Math.random().toString(36).slice(2,6),
      reason,
      createdAt:new Date().toLocaleString(),
      schoolCount:schoolCountOf(source),
      payload:clone(source)
    };
    let list=[];
    try{list=JSON.parse(localStorage.getItem(LOCAL_BACKUP_KEY)||"[]")}catch(e){}
    const last=list[0];
    const sig=JSON.stringify({s:item.schoolCount,r:(source.records||[]).length,first:source.schools[0]?.name||""});
    const lastSig=last?JSON.stringify({s:last.schoolCount,r:(last.payload.records||[]).length,first:last.payload.schools?.[0]?.name||""}):"";
    if(sig!==lastSig || reason.includes("导入") || reason.includes("云端")){
      list.unshift(item);
...
function exportCurrentData(){
  backupSnapshot("手动导出前备份");
  downloadJson(data,"超能游学报价系统数据备份_"+new Date().toISOString().slice(0,10)+".json");
}
function renderBackupManager(){
  const el=$("backupManager");
  if(!el) return;
  const list=localBackups();
  el.innerHTML=`<div class="sub-box"><h3>防丢备份中心</h3>
    <p class="muted">系统会在保存/上传云端前自动保留本地历史备份；云端上传前也会自动生成云端历史备份。避免再次被空数据覆盖。</p>
    <div class="btn-row">
      <button onclick="exportCurrentData()">下载当前数据备份</button>
      <button class="secondary" onclick="renderBackupManager()">刷新本地备份</button>
      <button class="secondary" onclick="loadCloudBackups()">查看云端历史备份</button>
    </div>
    <div style="margin-top:12px"><b>本地历史备份</b></div>
    ${list.length?list.slice(0,10).map(b=>`<div class="list-item"><div><b>${b.createdAt}</b><br/><span class="muted">${html(b.reason)}｜学校 ${b.schoolCount} 个</span></div><div class="list-actions"><button onclick="restoreLocalBackup('${b.id}')">恢复</button><button class="secondary" onclick="downloadLocalBackup('${b.id}')">下载</button></div></div>`).join(""):`<p class="muted">暂无本地备份。之后每次保存会自动生成。</p>`}
    <div id="cloudBackupList"></div>
  </div>`;
}
function restoreLocalBackup(id){
  const b=localBackups().find(x=>x.id===id);
  if(!b) return alert("没有找到这个备份");
  if(!confirm(`确定恢复这个本地备份？\\n时间：${b.createdAt}\\n学校：${b.schoolCount} 个\\n\\n恢复后会先写入本机，不会自动上传云端。确认无误后再手动上传云端。`)) return;
  backupSnapshot("恢复本地备份前");
  data=normalize(b.payload);
  saveLocalOnly();
  selectedSchoolId=data.schools[0]?.id;
  editingSchoolId=selectedSchoolId;
  refreshAll();
  alert("已恢复到本机。确认数据正确后，再到系统设置手动上传到云端。");
}
function downloadLocalBackup(id){
  const b=localBackups().find(x=>x.id===id);
  if(!b) return alert("没有找到这个备份");
  downloadJson(b.payload,`超能游学本地备份_${b.schoolCount}校_${String(b.createdAt).replace(/[\\/:\\s]/g,"-")}.json`);
}
async function loadCloudBackups(){
  const box=$("cloudBackupList");
  if(box) box.innerHTML="<p class='muted'>正在读取云端历史备份...</p>";
  try{
    const res=await fetch("/api/data?backups=1",{cache:"no-store",headers:authHeaders()});
    const body=await res.json().catch(()=>({}));
    if(!res.ok||body.error) throw new Error(body.error||("HTTP "+res.status));
    const backups=body.backups||[];
    if(box){
      box.innerHTML=`<div style="margin-top:14px"><b>云端历史备份</b></div>`+
      (backups.length?backups.map((r,i)=>{
        const p=r.payload||{};
        const real=p.payload||p;
        const count=p.schoolCount||schoolCountOf(real);
        const time=p.createdAt||r.updated_at||"";
        return `<div class="list-item"><div><b>${html(String(time).slice(0,19))}</b><br/><span class="muted">学校 ${count} 个｜${html(p.reason||"云端备份")}</span></div><div class="list-actions"><button onclick="restoreCloudBackup(${i})">恢复</button><button class="secondary" onclick="downloadCloudBackup(${i})">下载</button></div></div>`;
      }).join(""):`<p class="muted">云端暂无历史备份。以后每次上传云端前会自动生成。</p>`);
      window.__cloudBackups=backups;
    }
  }catch(err){
    if(box) box.innerHTML=`<p class="muted">读取失败：${html(err.message||err)}</p>`;
    alert("读取云端备份失败："+(err.message||err));
  }
}
function cloudBackupPayload(i){
  const r=(window.__cloudBackups||[])[i];
  if(!r) return null;
  const p=r.payload||{};
  return p.payload||p;
}
function restoreCloudBackup(i){
  const p=cloudBackupPayload(i);
  if(!p) return alert("没有找到这个云端备份");
  if(!confirm(`确定恢复云端备份到本机？\\n学校：${schoolCountOf(p)} 个\\n\\n恢复后只写入本机，不会自动上传云端。确认无误后再手动上传云端。`)) return;
  backupSnapshot("恢复云端备份前");
  data=normalize(p);
  saveLocalOnly();
  selectedSchoolId=data.schools[0]?.id;
  editingSchoolId=selectedSchoolId;
  refreshAll();
  alert("已恢复云端备份到本机。确认正确后，再手动上传到云端。");
}
function downloadCloudBackup(i){
  const p=cloudBackupPayload(i);
  if(!p) return alert("没有找到这个云端备份");
  downloadJson(p,`超能游学云端历史备份_${schoolCountOf(p)}校.json`);
}
...
function holidayWeeksForStay(startDate,weeks,d){
  if(!d || !d.holidayDiscountEnabled) return 0;
  let st=dateOnly(startDate), hs=dateOnly(d.holidayDiscountStart), he=dateOnly(d.holidayDiscountEnd);
  if(!st || !hs || !he || !weeks) return 0;
  let count=0;
  for(let i=0;i<weeks;i++){
    // 和旺季逻辑一致：周日入学当天不算，实际学习周按周一到周六判断。
    let ws=dateAddDays(st,i*7+1), we=dateAddDays(st,i*7+6);
    if(ws<=he && we>=hs) count++;
  }
  // 圣诞元旦优惠按1周/2周档位计算，最多按2周。
  return Math.min(2,count);
}
function holidayDiscountAmount(d,w){
  w=Math.max(0,num(w));
  if(!d || !d.holidayDiscountEnabled || w<=0) return 0;
  if(w>=2) return num(d.holidayDiscount2w) + num(d.holidayDiscount2wExtra);
  return num(d.holidayDiscount1w);
}
function holidayDiscountNote(d,w){
  if(!d || !d.holidayDiscountEnabled || w<=0) return "";
  let txt=`覆盖${w}周`;
  if(w>=2 && num(d.holidayDiscount2wExtra)>0) txt += "，含2周额外优惠";
  return txt;
}
...
function schoolInitialText(s){
  const raw=`${s?.name||""} ${s?.campus||""}`;
  const wordMap=[
    ["宿务","sw"],["碧瑶","by"],["克拉克","klk"],["伊洛伊洛","ylyl"],["马尼拉","mnl"],["长滩岛","ctd"],["长滩","ct"],
    ["巴科洛德","bkld"],["达沃","dw"],["薄荷","bh"],["怡朗","yl"],["菲律宾","flb"],["校区","xq"],["学校","xx"],
    ["半斯巴达","bsbd"],["斯巴达","sbd"],["主校区","zxq"],["城市校区","csxq"]
  ];
  let tags=[raw];
  wordMap.forEach(([k,v])=>{if(raw.includes(k))tags.push(v)});
  const latin=(raw.match(/[A-Za-z0-9]+/g)||[]);
  if(latin.length){
    tags.push(latin.join(""));
    tags.push(latin.map(x=>x[0]).join(""));
  }
  const upper=(raw.match(/[A-Z]{2,}/g)||[]);
  if(upper.length) tags.push(upper.join(""));
  return normalizedSearchText(tags.join(" "));
}
function filteredSchoolsBySearch(){
  const q=normalizedSearchText($("schoolSearch")?.value||"");
  if(!q) return data.schools;
  return data.schools.filter(s=>schoolInitialText(s).includes(q));
}
function renderSchoolOptions(){
  const ssel=$("schoolSelect");
  if(!ssel) return;
  const list=filteredSchoolsBySearch();
  const show=list.length?list:data.schools;
  if(!show.some(s=>s.id===selectedSchoolId)){
    selectedSchoolId=(show[0]||data.schools[0])?.id;
  }
  ssel.innerHTML=show.map(s=>`<option value="${s.id}">${s.name} ${s.campus}</option>`).join("");
  ssel.value=selectedSchoolId;
}

function roomDiscountRulesOf(d){
  const rules=Array.isArray(d?.roomDiscountRules)?d.roomDiscountRules:[];
  return rules.filter(r=>r && r.enabled!==false && num(r.amount)>0 && String(r.keywords||"").trim());
}
function roomRuleMatches(roomName,rule){
  const name=String(roomName||"").toLowerCase();
  const compact=normalizedSearchText(name);
  const keys=String(rule?.keywords||"").split(/[,，、;；\n]/).map(x=>x.trim()).filter(Boolean);
  if(!keys.length) return false;
  return keys.some(k=>{
    const kk=String(k).toLowerCase();
    return name.includes(kk) || compact.includes(normalizedSearchText(kk));
  });
}
function roomDiscountAmountByRule(rule,weeks){
  const amount=num(rule?.amount);
  const mode=rule?.mode||"per4";
  if(mode==="per1") return amount*num(weeks);
  if(mode==="fixed") return amount;
  return Math.floor(num(weeks)/4)*amount;
}
function roomDiscountForStay(d,roomDetails){
  if(!d?.roomDiscountEnabled) return {total:0,details:[]};
  const rules=roomDiscountRulesOf(d);
  let details=[],total=0;
  (roomDetails||[]).forEach(rd=>{
    const roomName=rd.item?.name||"";
    const rule=rules.find(r=>roomRuleMatches(roomName,r));
    if(!rule) return;
    const amount=roomDiscountAmountByRule(rule,rd.weeks);
    if(amount<=0) return;
    const modeText=(rule.mode||"per4")==="per1"?"按周":((rule.mode||"per4")==="fixed"?"固定":"按4周");
    details.push({roomName,ruleName:rule.name||"房型优惠",weeks:rd.weeks,amount,modeText});
    total+=amount;
  });
  return {total,details};
}
function roomDiscountDetailText(list){
  return (list||[]).map(x=>`${x.roomName} ${x.weeks}周 ${x.modeText}`).join("；");
}
...
function syncSeasonAutoRules(){
  ...
  let outsidePeakWeeks=Math.max(0,weeks-peakWeeks);
  let restore=!!d.peakRestoreDiscountsEnabled && outsidePeakWeeks>0;
  setCheck("peakSeason", inPeak);

  const lowDefault = inPeak ? ((!!d.peakAllowLowSeasonDiscount || restore) && d.lowSeasonEnabled!==false) : d.lowSeasonEnabled!==false;
  // 如果后台开启“淡季优惠前端可手动选择”，用户在自动报价页勾选/取消淡季优惠时，系统不再反复强制改回默认值。
  // 只有切换学校/初始化时，才按后台默认优惠设置一次。
  if(!d.lowSeasonManualSelectable || applyingSchoolDefaults){
    setCheck("lowSeason", lowDefault);
  }

  setCheck("schoolRate", inPeak ? ((!!d.peakAllowSchoolRateDiscount || restore) && !!d.schoolLowSeasonRateEnabled) : !!d.schoolLowSeasonRateEnabled);
  setCheck("longDiscount", inPeak ? ((!!d.peakAllowLongDiscount || restore) && d.longDiscountEnabled!==false) : d.longDiscountEnabled!==false);
  setCheck("agencyDiscount", inPeak ? ((!!d.peakAllowAgencyDiscount || restore) && d.agencyDiscountEnabled!==false) : d.agencyDiscountEnabled!==false);
  setCheck("roomDiscount", !!d.roomDiscountEnabled);
  const holidayWeeks=holidayWeeksForStay(start,weeks,d);
  setCheck("holidayDiscount", !!d.holidayDiscountEnabled && holidayWeeks>0);
  setCheck("waiveRegistration", inPeak ? ((!!d.peakAllowRegistrationWaiver || restore) && !!d.registrationWaiverEnabled) : !!d.registrationWaiverEnabled);
  return {peakWeeks,inPeak,outsidePeakWeeks,restore,holidayWeeks};
}
function lowSeasonAmount(d,w){
  w=Math.max(0,num(w));
  if((d.lowSeasonDiscountMode||"per4")==="per1"){
    return num(d.lowSeasonDiscountPer1w)*w;
  }
  return Math.floor(w/4)*num(d.lowSeasonDiscountPer4w);
}
function longAmount(d,w){if(w>=24)return num(d.long24);if(w>=20)return num(d.long20);if(w>=16)return num(d.long16);if(w>=12)return num(d.long12);if(w>=8)return num(d.long8);if(w>=4)return num(d.long4);return 0}
function isMultiMode(){
  return !!$("multiMode") && $("multiMode").value==="true";
}
function optionText(list,id){
  let item=(list||[]).find(x=>x.id===id)||list?.[0];
  return item||{};
}
...
function buildSelectionDetails(primary,secondary,w1,w2,kind){
  let arr=[];
  if(w1>0) arr.push({item:primary,weeks:w1,amount:num(primary?.price4w)*w1/4,label:kind+"1"});
  if(w2>0) arr.push({item:secondary,weeks:w2,amount:num(secondary?.price4w)*w2/4,label:kind+"2"});
  return arr;
}
function detailName(details){
  return details.map(d=>`${d.item?.name||"-"}${details.length>1?`（${d.weeks}周）`:""}`).join(" + ");
}
function courseDetailsText(details){
  return details.map(d=>`${d.item?.name||"-"}：${courseLesson(d.item)||d.weeks+"周"}`).join("；");
}
function roomDetailsText(details){
  return details.map(d=>`${d.item?.name||"-"} ${d.weeks}周`).join("；");
}
function discountWeeksByRule(d,weeks,peakWeeks,allowInPeak,baseEnabled){
  if(!baseEnabled) return 0;
  weeks=Math.max(0,num(weeks));
  peakWeeks=Math.max(0,num(peakWeeks));
  let inPeak=peakWeeks>0 && !!d.peakSeasonEnabled;
  if(!inPeak) return weeks;

  // 旺季覆盖期间和旺季外期间分开算：
  // 1. 如果“旺季期间仍启用某优惠”开启，这个优惠只对旺季覆盖周数生效。
  // 2. 如果“旺季外时间自动恢复优惠”开启，这个优惠再对旺季外周数生效。
  // 3. 不会再从入学总周数直接计算，避免把旺季时间也算进淡季/长期/学校折扣。
  let eligible=0;
  if(allowInPeak) eligible += peakWeeks;
  if(d.peakRestoreDiscountsEnabled) eligible += Math.max(0,weeks-peakWeeks);
  return Math.min(weeks,eligible);
}
...
function applySchoolDefaults(){
  applyingSchoolDefaults=true;
  syncSeasonAutoRules();
  applyingSchoolDefaults=false;
}
...
function calc(){
  let school=currentSchool(),
      ...
      course2=school.courses.find(c=>c.id===$("courseSelect2")?.value)||school.courses[1]||school.courses[0],
      room=school.rooms.find(r=>r.id===$("roomSelect").value)||school.rooms[0],
      room2=school.rooms.find(r=>r.id===$("roomSelect2")?.value)||school.rooms[1]||school.rooms[0],
      weeks=num($("weeksSelect").value||4),
      d=school.discounts||{};

  let [cw1,cw2]=splitWeeks("course",weeks);
  let [rw1,rw2]=splitWeeks("room",weeks);
  let courseDetails=buildSelectionDetails(course,course2,cw1,cw2,"课程");
  let roomDetails=buildSelectionDetails(room,room2,rw1,rw2,"住宿");

  let autoSeason=syncSeasonAutoRules();
  let tuition=courseDetails.reduce((a,b)=>a+num(b.amount),0),
      dorm=roomDetails.reduce((a,b)=>a+num(b.amount),0),
      base=tuition+dorm,
      peak=$("peakSeason").checked?num(d.peakFeePerWeek)*autoSeason.peakWeeks:0;

  let lowWeeks=discountWeeksByRule(d,weeks,autoSeason.peakWeeks,!!d.peakAllowLowSeasonDiscount,$("lowSeason").checked && d.lowSeasonEnabled!==false);
  let longWeeks=discountWeeksByRule(d,weeks,autoSeason.peakWeeks,!!d.peakAllowLongDiscount,$("longDiscount").checked && d.longDiscountEnabled!==false);
  let schoolRateWeeks=discountWeeksByRule(d,weeks,autoSeason.peakWeeks,!!d.peakAllowSchoolRateDiscount,$("schoolRate").checked && !!d.schoolLowSeasonRateEnabled);
  let agencyWeeks=discountWeeksByRule(d,weeks,autoSeason.peakWeeks,!!d.peakAllowAgencyDiscount,$("agencyDiscount").checked && d.agencyDiscountEnabled!==false);
  let waiveWeeks=discountWeeksByRule(d,weeks,autoSeason.peakWeeks,!!d.peakAllowRegistrationWaiver,$("waiveRegistration").checked && !!d.registrationWaiverEnabled);

  let low=lowSeasonAmount(d,lowWeeks),
      longD=longAmount(d,longWeeks),
      holidayWeeks=autoSeason.holidayWeeks || holidayWeeksForStay($("startDateSelect").value,weeks,d),
      holidayD=($("holidayDiscount")?.checked && !!d.holidayDiscountEnabled) ? holidayDiscountAmount(d,holidayWeeks) : 0,
      roomDiscountInfo=($("roomDiscount")?.checked && !!d.roomDiscountEnabled) ? roomDiscountForStay(d,roomDetails) : {total:0,details:[]},
      roomD=roomDiscountInfo.total;

  let beforeSchool=base-low-longD-holidayD-roomD+peak;

  let schoolEligibleBase=Math.max(0,base*(schoolRateWeeks/weeks)-low-longD-holidayD-roomD);
  let schoolRate=schoolRateWeeks>0 ? schoolEligibleBase*(1-num(d.schoolLowSeasonDiscountRate)) : 0;
  ...
  return{school,course,course2,room,room2,courseDetails,roomDetails,courseName:detailName(courseDetails),roomName:detailName(roomDetails),courseDetailText:courseDetailsText(courseDetails),roomDetailText:roomDetailsText(roomDetails),multiMode:isMultiMode(),weeks,startDate:$("startDateSelect").value,endDate:addWeeks($("startDateSelect").value,weeks),tuition,dorm,lowDiscount:low,lowDiscountWeeks:lowWeeks,lowDiscountMode:d.lowSeasonDiscountMode||"per4",peakWeeks:autoSeason.peakWeeks,outsidePeakWeeks:autoSeason.outsidePeakWeeks,inPeak:autoSeason.inPeak,restoreDiscounts:autoSeason.restore,peakFee:peak,longDiscount:longD,longDiscountWeeks:longWeeks,holidayDiscount:holidayD,holidayDiscountWeeks:holidayWeeks,holidayDiscountNote:holidayDiscountNote(d,holidayWeeks),roomDiscount:roomD,roomDiscountDetails:roomDiscountInfo.details,roomDiscountNote:roomDiscountDetailText(roomDiscountInfo.details),schoolLowSeasonDiscount:schoolRate,schoolRateWeeks,agencyDiscount:agency,agencyDiscountWeeks:agencyWeeks,agencyIncludesPeak:agencyPeakPart>0,registrationWaiver:regWaive,registrationWaiverWeeks:waiveWeeks,registrationFee:regFee,originalRegistrationFee:num(d.registrationFee),totalUsd,localItems,localPeso,excludedLocalPeso:localItems.reduce((a,b)=>a+(b.excluded?num(b.total):0),0),tuitionRmb,localRmb,totalRmb:tuitionRmb+localRmb}
}
function renderSelectors(){
  renderSchoolOptions();
  let s=currentSchool();

  let oldCourse=$("courseSelect").value, oldCourse2=$("courseSelect2")?.value;
  $("courseSelect").innerHTML=s.courses.map(c=>`<option value="${c.id}">${c.name}</option>`).join("");
  if(oldCourse) $("courseSelect").value=oldCourse;
  if($("courseSelect2")){
    $("courseSelect2").innerHTML=s.courses.map(c=>`<option value="${c.id}">${c.name}</option>`).join("");
    $("courseSelect2").value=oldCourse2||s.courses[1]?.id||s.courses[0]?.id||"";
  }

  let oldRoom=$("roomSelect").value, oldRoom2=$("roomSelect2")?.value;
  $("roomSelect").innerHTML=s.rooms.map(r=>`<option value="${r.id}">${r.name}</option>`).join("");
  if(oldRoom) $("roomSelect").value=oldRoom;
  if($("roomSelect2")){
    $("roomSelect2").innerHTML=s.rooms.map(r=>`<option value="${r.id}">${r.name}</option>`).join("");
    $("roomSelect2").value=oldRoom2||s.rooms[1]?.id||s.rooms[0]?.id||"";
  }

  let oldW=$("weeksSelect").value||4;
  $("weeksSelect").innerHTML=Array.from({length:24},(_,i)=>i+1).map(w=>`<option value="${w}">${w}周</option>`).join("");
  $("weeksSelect").value=oldW;
  let oldD=$("startDateSelect").value;
  $("startDateSelect").innerHTML=sundayOptions().map(v=>`<option value="${v}">${v} 周日</option>`).join("");
  if(oldD)$("startDateSelect").value=oldD;
  updateMultiModeUI();
  applySchoolDefaults();
}function line(a,b,cls=""){return `<div class="calc-line ${cls}"><span>${a}</span><strong>${b}</strong></div>`}function renderCalc(){let c=calc();let rows=`${line("课程学费",money(c.tuition))}${line("住宿费",money(c.dorm))}`;if(num(c.lowDiscount)>0)rows+=line(`淡季优惠（${c.restoreDiscounts?"旺季外恢复，":""}${c.lowDiscountMode==="per1"?"按周":"按4周"}，计${c.lowDiscountWeeks}周）`,"-"+money(c.lowDiscount),"minus");if(num(c.longDiscount)>0)rows+=line(`长期优惠（${c.restoreDiscounts?"旺季外恢复，":""}计${c.longDiscountWeeks}周）`,"-"+money(c.longDiscount),"minus");if(num(c.holidayDiscount)>0)rows+=line(`圣诞元旦优惠（${c.holidayDiscountNote||"自动匹配"}）`,"-"+money(c.holidayDiscount),"minus");if(num(c.roomDiscount)>0)rows+=line(`房型专属优惠（${c.roomDiscountNote||"自动匹配"}）`,"-"+money(c.roomDiscount),"minus");if(num(c.schoolLowSeasonDiscount)>0)rows+=line("学校淡季折扣","-"+money(c.schoolLowSeasonDiscount),"minus");if(num(c.peakFee)>0)rows+=line(`旺季附加（覆盖${c.peakWeeks}周）`,money(c.peakFee));if(num(c.agencyDiscount)>0)rows+=line(`${agencyDiscountLabel()}${c.agencyIncludesPeak?"（含旺季附加）":""}`,"-"+money(c.agencyDiscount),"minus");if(num(c.registrationWaiver)>0)rows+=line(agencyWaiverLabel(),"-"+money(c.registrationWaiver),"minus");rows+=line("注册金",money(c.registrationFee));$("calcBox").innerHTML=`${rows}<div class="calc-total"><span>费用一合计：${money(c.totalUsd)} / ${rmb(c.tuitionRmb)}</span><span>费用二合计：${peso(c.localPeso)} / ${rmb(c.localRmb)}</span><span class="local-note">宿舍押金、接机费不含在学杂费合计内。</span><b>总人民币：${rmb(c.totalRmb)}</b></div>`;renderQuoteSheet()}
...
function buildUsdRows(c,s){
  ...
  if(num(c.lowDiscount)>0) rows.push(["淡季优惠",c.lowDiscountMode==="per1"?`${s.discounts.lowSeasonDiscountPer1w||0}美元/周 × ${c.lowDiscountWeeks}周`:`${s.discounts.lowSeasonDiscountPer4w}美元/4周，计${c.lowDiscountWeeks}周`,`-${Math.round(c.lowDiscount).toLocaleString()}`,c.restoreDiscounts?"旺季外恢复":"自动匹配"]);
  if(num(c.longDiscount)>0) rows.push(["长期优惠",`${c.longDiscountWeeks}周对应`,`-${Math.round(c.longDiscount).toLocaleString()}`,c.restoreDiscounts?"旺季外恢复":"自动匹配"]);
  if(num(c.holidayDiscount)>0) rows.push(["圣诞元旦优惠",`${c.holidayDiscountNote||"自动匹配"}`,`-${Math.round(c.holidayDiscount).toLocaleString()}`,"自动匹配"]);
  if(num(c.roomDiscount)>0) rows.push(["房型专属优惠",`${c.roomDiscountNote||"自动匹配"}`,`-${Math.round(c.roomDiscount).toLocaleString()}`,"自动匹配"]);
  ...
  if(num(c.agencyDiscount)>0) rows.push([agencyDiscountLabel(),`折扣比例 ${s.discounts.agencyDiscountRate}${c.agencyIncludesPeak?"（含旺季附加）":""}`,`-${Math.round(c.agencyDiscount).toLocaleString()}`,agencyDiscountRemark()]);
...
function ensureImagePreviewModal(){
  ...
  modal.innerHTML=`<div class="image-preview-card">
    ...
    <p class="muted">手机端和电脑端使用同一张PNG规格。手机可点“打开同款PNG长按保存”，电脑会自动下载。</p>
...
async function saveCanvasImage(canvas, filename){
  ...
  try{
    // 手机和电脑统一使用同一个 drawNativeQuoteCanvas 输出，不再分手机模板/电脑模板。
    ...
    down.href=isMobile?dataUrl:blobUrl;
    down.download=safeName;
    down.textContent=isMobile?"打开/保存同款PNG":"下载图片";

    openBtn.textContent=isMobile?"打开同款PNG长按保存":"打开原图";
    openBtn.onclick=()=>{
      const w=window.open();
      const src=isMobile?dataUrl:blobUrl;
      if(w){
        w.document.write(`<title>${safeName}</title><meta name="viewport" content="width=device-width,initial-scale=1"><img src="${src}" style="width:100%;height:auto;display:block;background:#fff;-webkit-touch-callout:default;user-select:auto;">`);
        w.document.close();
      }else{
        location.href=src;
      }
...
async function drawNativeQuoteCanvas(){
  ...
  const heroH=138;
  const infoCardH=106;
  const usdCols=[(430)*.22,(430)*.40,(430)*.18,(430)*.20];
  const phpCols=[(430)*.40,(430)*.21,(430)*.17,(430)*.22];
...
${set.agencyAdvantageLine2}`,420,10).length
  ...
  const promoH=Math.max(86,50+promoTextLines*18);

  const courseLines=c.courseDetails.reduce((n,d)=>n+1+wrapCanvasText(temp,courseLesson(d.item)||"以学校安排为准",550,5).length,0);
  const roomLines=Math.max(1,c.roomDetails.length);
  const courseBoxH=Math.max(78,48+courseLines*18);
  const roomBoxH=Math.max(78,48+roomLines*22);
  ...
  const H = 20 + heroH + 18 + infoCardH + 16 + extraH + 14 + promoH + 16 + panelH + 16 + summaryH + 14 + footerH + 20;
  ...
  ctx.fillStyle=grad;canvasRoundRect(ctx,margin,20,W-margin*2,heroH,22,true,false);
  if(logo)ctx.drawImage(logo,margin+18,33,102,102);
  ctx.fillStyle="#0639a6";ctx.font="bold 38px Arial, sans-serif";
  drawWrap(ctx,`${s.name} ${s.campus}`,margin+145,66,760,44,1);
  ctx.font="bold 28px Arial, sans-serif";ctx.fillStyle="#111b63";
  ctx.fillText(`游学报价单（${c.weeks}周）`,margin+145,110);
  ctx.fillStyle="#0798e8";canvasRoundRect(ctx,margin+145,118,330,34,17,true,false);
  ctx.fillStyle="#fff";ctx.font="bold 19px Arial, sans-serif";ctx.fillText(quoteSlogan(),margin+168,141);
  ...
  function infoCard(x,y,w,h,title,main,sub){
    ...
    ctx.fillStyle="#111b63";ctx.font="bold 17px Arial, sans-serif";drawWrap(ctx,main,x+14,y+56,w-28,20,2);
    ctx.fillStyle="#667395";ctx.font="13px Arial, sans-serif";drawWrap(ctx,sub,x+14,y+h-36,w-28,16,2);
  }
  ...
  y+=infoCardH+16;
  const leftW=610, rightW=W-margin*2-leftW-gap;
  ctx.fillStyle="#fff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,margin,y,leftW,extraH,18,true,true);
  ctx.fillStyle="#0639a6";ctx.font="bold 20px Arial, sans-serif";ctx.fillText("具体课程课时",margin+16,y+28);
  let cy=y+54;
  c.courseDetails.forEach(d=>{
    ...
    cy+=drawWrap(ctx,courseLesson(d.item)||"以学校安排为准",margin+30,cy,leftW-44,17,5);
    cy+=4;
  });

  ctx.fillStyle="#fff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,margin+leftW+gap,y,rightW,extraH,18,true,true);
  ctx.fillStyle="#0639a6";ctx.font="bold 20px Arial, sans-serif";ctx.fillText("住宿安排",margin+leftW+28,y+28);
  let ry=y+54;
  ...
  ctx.fillStyle="#e4251a";ctx.font="bold 20px Arial, sans-serif";ctx.fillText(s.discounts.schoolPromoTitle,margin+16,y+28);
  ctx.fillStyle="#17214d";ctx.font="15px Arial, sans-serif";
  drawWrap(ctx,s.discounts.schoolPromoText,margin+16,y+54,leftW-32,18,12);

  ctx.fillStyle="#f7f9ff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,margin+leftW+gap,y,rightW,promoH,18,true,true);
  ctx.fillStyle="#0639a6";ctx.font="bold 20px Arial, sans-serif";ctx.fillText(set.agencyAdvantageTitle,margin+leftW+28,y+28);
  ctx.fillStyle="#17214d";ctx.font="15px Arial, sans-serif";
  drawWrap(ctx,`✓ ${set.agencyAdvantageLine1}
✓ ${set.agencyAdvantageLine2}`,margin+leftW+28,y+54,rightW-32,18,8);
  ...
  ctx.fillStyle="#fbfdff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,margin,y,W-margin*2,summaryH,22,true,true);
  ctx.fillStyle="#0639a6";ctx.font="bold 26px Arial, sans-serif";ctx.textAlign="center";ctx.fillText("本次游学总计（以实际汇率为准）",W/2,y+38);ctx.textAlign="left";
  function sumBox(x,title,sub,val,color){
    ctx.fillStyle="#fff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,x,y+56,250,82,16,true,true);
    ctx.fillStyle=color;ctx.font="bold 17px Arial, sans-serif";ctx.fillText(title,x+14,y+82);
    ctx.fillStyle="#17214d";ctx.font="13px Arial, sans-serif";drawWrap(ctx,sub,x+14,y+102,222,15,2);
    ctx.fillStyle=color;ctx.font="bold 17px Arial, sans-serif";ctx.fillText(val,x+14,y+128);
  }
  sumBox(margin+18,"美元部分",`${Math.round(c.totalUsd).toLocaleString()} USD × ${set.usdRate}`,rmb(c.tuitionRmb),"#0639a6");
  ctx.fillStyle="#0639a6";ctx.font="bold 34px Arial, sans-serif";ctx.fillText("+",margin+288,y+106);
  sumBox(margin+320,"披索部分",`${Math.round(c.localPeso).toLocaleString()} PHP × ${set.pesoRate}`,rmb(c.localRmb),"#0b7a48");
  ctx.fillStyle="#0639a6";ctx.font="bold 34px Arial, sans-serif";ctx.fillText("≈",margin+588,y+106);
  ctx.fillStyle="#fff";ctx.strokeStyle="#dbe7ff";canvasRoundRect(ctx,margin+640,y+56,240,82,16,true,true);
  ctx.fillStyle="#4a5b7d";ctx.font="bold 16px Arial, sans-serif";ctx.fillText("总人民币",margin+658,y+82);
  ctx.fillStyle="#e4251a";ctx.font="bold 28px Arial, sans-serif";ctx.fillText(`${Math.round(c.totalRmb).toLocaleString()} 元`,margin+658,y+116);
  ctx.fillStyle="#17214d";ctx.font="14px Arial, sans-serif";ctx.fillText("学费 + 本地费用",margin+658,y+136);

  y+=summaryH+14;
  ctx.fillStyle="#fff7e8";ctx.strokeStyle="#ffd98c";canvasRoundRect(ctx,margin,y,W-margin*2,footerH,18,true,true);
  ctx.fillStyle="#17214d";ctx.font="bold 16px Arial, sans-serif";ctx.fillText(`选择 ${set.brandName}｜价格透明｜专业顾问｜安心服务`,margin+18,y+24);
  ctx.font="14px Arial, sans-serif";drawWrap(ctx,"备注：菲律宾本地费用只做参考，最终以学校实际收取为准。宿舍押金、接机费不含。",margin+18,y+44,W-margin*2-36,16,2);
...
async function saveSchoolEditor(){let s=editingSchool();s.name=$("edName").value;s.campus=$("edCampus").value;Object.assign(s.discounts,{registrationFee:num($("edReg").value),lowSeasonDiscountPer4w:num($("edLow").value),lowSeasonDiscountPer1w:num($("edLow1w").value),lowSeasonDiscountMode:$("edLowMode").value,schoolLowSeasonDiscountRate:num($("edSchoolRate").value),peakFeePerWeek:num($("edPeak").value),agencyDiscountRate:num($("edAgency").value),registrationWaiverAmount:num($("edWaive").value),long4:num($("edLong4").value),long8:num($("edLong8").value),long12:num($("edLong12").value),long16:num($("edLong16").value),long20:num($("edLong20").value),long24:num($("edLong24").value),schoolPromoTitle:$("edPromoTitle").value,schoolPromoText:$("edPromoText").value,lowSeasonEnabled:$("edLowEnabled").value==="true",lowSeasonManualSelectable:$("edLowManual").value==="true",schoolLowSeasonRateEnabled:$("edSchoolRateEnabled").value==="true",peakSeasonEnabled:$("edPeakEnabled").value==="true",peakPeriods:[{start:$("edPeak1Start").value,end:$("edPeak1End").value},{start:$("edPeak2Start").value,end:$("edPeak2End").value}],peakRestoreDiscountsEnabled:$("edPeakRestore").value==="true",peakAllowLowSeasonDiscount:$("edPeakAllowLow").value==="true",peakAllowSchoolRateDiscount:$("edPeakAllowSchoolRate").value==="true",peakAllowLongDiscount:$("edPeakAllowLong").value==="true",peakAllowAgencyDiscount:$("edPeakAllowAgency").value==="true",peakAllowRegistrationWaiver:$("edPeakAllowWaive").value==="true",longDiscountEnabled:$("edLongEnabled").value==="true",agencyDiscountEnabled:$("edAgencyEnabled").value==="true",registrationWaiverEnabled:$("edWaiveEnabled").value==="true",holidayDiscountEnabled:$("edHolidayEnabled").value==="true",holidayDiscountStart:$("edHolidayStart").value,holidayDiscountEnd:$("edHolidayEnd").value,holidayDiscount1w:num($("edHoliday1w").value),holidayDiscount2w:num($("edHoliday2w").value),holidayDiscount2wExtra:num($("edHoliday2wExtra").value),roomDiscountEnabled:$("edRoomDiscountEnabled").value==="true",roomDiscountRules:collectRoomDiscountRulesFromEditor()});s.courses=s.courses.map((c,i)=>({id:c.id,name:$(`cName${i}`).value,price4w:num($(`cPrice${i}`).value),note:$(`cNote${i}`).value,lessonText:$(`cLesson${i}`).value}));s.rooms=s.rooms.map((r,i)=>({id:r.id,name:$(`rName${i}`).value,price4w:num($(`rPrice${i}`).value)}));saveLocalOnly();refreshAll();activateTab("schools");let ok=await uploadCloudData(true);if(ok){alert("已保存并同步到云端，手机端刷新后会更新")}else{alert("已保存到本机，但云端同步失败。请到系统设置点“测试云端连接”，确认 Vercel 的 ADMIN_PASSWORD 和你登录密码一致。")}}
