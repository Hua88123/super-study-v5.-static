...
function line(a,b,cls=""){return `<div class="calc-line ${cls}"><span>${a}</span><strong>${b}</strong></div>`}function renderCalc(){let c=calc();$("calcBox").innerHTML=`${line("课程学费",money(c.tuition))}${line("住宿费",money(c.dorm))}${line("淡季优惠","-"+money(c.lowDiscount),"minus")}${line("长期优惠","-"+money(c.longDiscount),"minus")}${line("学校淡季折扣","-"+money(c.schoolLowSeasonDiscount),"minus")}${line("旺季附加",money(c.peakFee))}${line("超能折扣","-"+money(c.agencyDiscount),"minus")}${line("超能减免注册金","-"+money(c.registrationWaiver),"minus")}${line("注册金",money(c.registrationFee))}<div class="calc-total"><span>费用一合计：${money(c.totalUsd)} / ${rmb(c.tuitionRmb)}</span><span>费用二合计：${peso(c.localPeso)} / ${rmb(c.localRmb)}</span><b>总人民币：${rmb(c.totalRmb)}</b></div>`;renderQuoteSheet()}
...
function renderFeeEditor(){let s=editingSchool();$("feeEditor").innerHTML=`<h3>${s.name} ${s.campus}</h3><div class="editor-grid">${s.localFees.map((f,i)=>`${field(f.name,`fee${i}`,f.amount,"number")}<label>计算方式<select id="feeType${i}"><option value="fixed" ${!f.perWeek?"selected":""}>固定费用</option><option value="week" ${f.perWeek?"selected":""}>按周计算</option></select></label>`).join("")}</div><div class="sub-box"><h3>官方 Total 覆盖（可选）</h3><p class="muted">如果学校给了某个周数的本地费用总额，可以填这里。填 0 表示不用覆盖。</p><div class="editor-grid">${[4,8,12,16,20,24].map(w=>field(`${w}周 Total`,`total${w}`,s.officialTotals?.[w]||0,"number")).join("")}</div></div><button onclick="saveFees()">保存本地费用</button>`}function saveFees(){let s=editingSchool();s.localFees=s.localFees.map((f,i)=>({...f,amount:num($(`fee${i}`).value),perWeek:$(`feeType${i}`).value==="week"}));[4,8,12,16,20,24].forEach(w=>{s.officialTotals[w]=num($(`total${w}`).value)});saveData();renderCalc();alert("已保存")}
function renderSettings(){let set=data.settings;$("settingsEditor").innerHTML=`<div class="editor-grid">${field("品牌中文","setBrand",set.brandName)}${field("品牌英文","setBrandEn",set.brandEn)}${field("美金汇率","setUsd",set.usdRate,"number")}${field("披索汇率","setPeso",set.pesoRate,"number")}${field("水印文字","setWatermark",set.watermarkText)}<label>是否开启水印<select id="setWatermarkEnabled"><option value="true" ${set.watermarkEnabled?"selected":""}>开启</option><option value="false" ${!set.watermarkEnabled?"selected":""}>关闭</option></select></label>${field("优势标题","setAdvTitle",set.agencyAdvantageTitle)}${field("优势1","setAdv1",set.agencyAdvantageLine1)}${field("优势2","setAdv2",set.agencyAdvantageLine2)}${field("管理员密码","setAdminPassword",set.adminPassword||"SuperStudy888","text","员工不知道这个密码就只能看到自动报价")}<label>上传 Logo<input type="file" id="logoUpload" accept="image/*"/></label></div><img class="logo-preview" src="${set.brandLogo}"/><div class="btn-row"><button onclick="saveSettings()">保存系统设置</button></div>`;$("logoUpload").addEventListener("change",e=>{let file=e.target.files[0];if(!file)return;let r=new FileReader();r.onload=()=>{data.settings.brandLogo=r.result;saveData();refreshAll()};r.readAsDataURL(file)})}function saveSettings(){let set=data.settings;set.brandName=$("setBrand").value;set.brandEn=$("setBrandEn").value;set.usdRate=num($("setUsd").value);set.pesoRate=num($("setPeso").value);set.watermarkText=$("setWatermark").value;set.watermarkEnabled=$("setWatermarkEnabled").value==="true";set.agencyAdvantageTitle=$("setAdvTitle").value;set.agencyAdvantageLine1=$("setAdv1").value;set.agencyAdvantageLine2=$("setAdv2").value;set.adminPassword=$("setAdminPassword").value||"SuperStudy888";saveData();refreshAll();alert("已保存")}
