...
function calc(){
  ...
  // 专属折扣：如果只适用于旺季外，也只按旺季外可优惠周数对应基数计算。
  let agencyEligibleBase=Math.max(0,base*(agencyWeeks/weeks)-low-longD-schoolRate);
  let agency=agencyWeeks>0 ? agencyEligibleBase*(1-num(d.agencyDiscountRate)) : 0;
  ...
  return{school,course,room,weeks,startDate:$("startDateSelect").value,endDate:addWeeks($("startDateSelect").value,weeks),tuition,dorm,lowDiscount:low,lowDiscountWeeks:lowWeeks,lowDiscountMode:d.lowSeasonDiscountMode||"per4",peakWeeks:autoSeason.peakWeeks,outsidePeakWeeks:autoSeason.outsidePeakWeeks,inPeak:autoSeason.inPeak,restoreDiscounts:autoSeason.restore,peakFee:peak,longDiscount:longD,longDiscountWeeks:longWeeks,schoolLowSeasonDiscount:schoolRate,schoolRateWeeks,agencyDiscount:agency,agencyDiscountWeeks:agencyWeeks,registrationWaiver:regWaive,registrationWaiverWeeks:waiveWeeks,registrationFee:regFee,originalRegistrationFee:num(d.registrationFee),totalUsd,localItems,localPeso,excludedLocalPeso:localItems.reduce((a,b)=>a+(b.excluded?num(b.total):0),0),tuitionRmb,localRmb,totalRmb:tuitionRmb+localRmb}
}
